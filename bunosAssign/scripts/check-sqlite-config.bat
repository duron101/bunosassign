...
echo 🔍 SQLite 数据库配置检查脚本
echo ==============================
echo.

echo [INFO] 检查 SQLite 配置...
echo.
...
echo.
echo try {
echo   const sqlite3 = require('sqlite3'^);
echo   console.log('✅ SQLite3模块加载成功'^);
echo } catch (e^) {
echo   console.error('❌ SQLite3模块加载失败:', e.message^);
echo   process.exit(1^);
echo }
echo.
echo // 加载环境变量
...
echo   console.log('✅ SQLite 数据库连接成功'^);
echo   console.log('   数据库文件:', dbPath^);
echo   console.log('   数据库类型:', 'SQLite'^);
echo   process.exit(0^);
echo }^).catch(err =^> {
echo   console.error('❌ SQLite 数据库连接失败:'^);
