/**
 * RBAC权限系统安全测试 - 独立测试（不依赖数据库）
 * 测试权限验证逻辑的安全性和正确性
 */

// 直接引入模块，避免Jest setup问题
const path = require('path')
const permissionsPath = path.join(__dirname, '../../config/permissions.js')
const { PermissionValidator, PERMISSIONS, ROLE_PERMISSIONS } = require(permissionsPath)

describe('RBAC Security Tests - Standalone', () => {
  ...
  describe('PermissionValidator.hasPermission', () => {
    ...
    test('应该安全处理无效输入', () => {
      ...
      expect(PermissionValidator.hasPermission(['user:view'], undefined)).toBe(false)
    })
  })
  ...
  describe('边界条件测试', () => {
    test('应该处理大量权限数组', () => {
      ...
      for (let i = 0; i < 1000; i++) { // 减少到1000避免内存问题
      ...
      expect(PermissionValidator.hasPermission(largePermissionArray, 'resource500:action500')).toBe(true)
    })

    test('应该处理极长的权限字符串', () => {
      const longPermission = 'a'.repeat(100) + ':' + 'b'.repeat(100) // 减少长度避免性能问题
